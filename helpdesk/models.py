...
class Queue(models.Model):
    ...
    plant = models.ForeignKey(
        SolarPlant, on_delete=models.CASCADE,
        related_name="queues"
    )
            ...
            # pass a list of identifiers and a dict with key as an identifier and a list with error codes
                ...
                try:
                    ...
                    if len(identifiers_list) > 0:
                        for key in list(alarms_dict.keys()):
            ...
            for ticket in tickets:
                try:
                    if event_types and ticket.event_type not in event_types:
                        continue
                    # holds one ticket of each type per plant rule
                    assert(ticket.event_type != list(data.keys()))
                except AssertionError as exc:
...
class TicketAssociation(models.Model):
    ticket = models.ForeignKey(Ticket, on_delete=models.CASCADE, related_name="associations",
...
class AlarmAssociation(BaseEventAssociation):
    ...
    ticket_association = models.ForeignKey(TicketAssociation, on_delete=models.CASCADE, related_name="alarms",
...
class AnalyticsAssociation(BaseEventAssociation):
    # TODO add a unique for a combination of ticket_association, and identifier
    ticket_association = models.ForeignKey(TicketAssociation, on_delete=models.CASCADE,
...
class KBCategory(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('helpdesk_kb_category', (), {'slug': self.slug})
...
class KBItem(models.Model):
    ...
    category = models.ForeignKey(
        KBCategory, on_delete=models.CASCADE,
    ...
    def get_absolute_url(self):
        return reverse('helpdesk_kb_item', (self.id,))
    # get_absolute_url = models.permalink(get_absolute_url)
...
class SLADefinition(models.Model):
    ...
    queue = models.ForeignKey(Queue, on_delete=models.CASCADE, related_name='slaqueue', related_query_name='slaqueue')
