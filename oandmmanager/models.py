...
class Tasks(models.Model):
    ...
    def serialize():
        ...
        for task in Tasks.objects.all().order_by('id'):
            if task.name not in list(tasks.keys()):
    ...
    def validate(preferences):
        tasks = []
        for task in list(preferences.keys()):
            task_name = task
...
class Cycle(models.Model):
    ...
    def tasks_summary(self):
        ...
        for task_name in list(preferences.keys()):
            try:
    ...
    def completion_summary(self, task_name):
        ...
        for task in self.tasks.filter(task__in=Tasks.objects.filter(name=task_name)):
            ...
            if task_summary is not None:
                ...
                if task_range not in list(summary[time].keys()):
        ...
        for time in list(summary.keys()):
            info = {"time": time}
            for task_range in list(summary[time].keys()):
                info[task_range] = summary[time][task_range]
                if task_range not in list(headers_keys.keys()):
...
class TaskItem(models.Model):
    cycle = models.ForeignKey(Cycle, on_delete=models.CASCADE, related_name="cycles", related_query_name="cycles")
    task = models.ForeignKey(Tasks, on_delete=models.CASCADE, related_name="tasks", related_query_name="tasks")
    ...
    assigned_to = models.ForeignKey(User, on_delete=models.CASCADE, related_name="users", related_query_name="users")
...
class TaskAssociation(models.Model):
    task_item = models.ForeignKey(TaskItem, on_delete=models.CASCADE, related_name="task_associations", related_query_name="task_associations")
    sensor = models.ForeignKey(Sensor, on_delete=models.CASCADE, related_name="sensors", related_query_name="sensors")
