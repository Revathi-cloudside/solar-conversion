...
import datetime, http.client, logging, sys
            ...
            try:
                try:
                    ...
                    if source.dataFormat == 'CSV':
                        ...
                        for stream_name in list(data_values.keys()):
        ...
        if source.actuationEnabled:
            try:
                ...
                if source.textMessageWithHTTP200:
                    response = JsonResponse({'action': ack_flag,'config': config_flag, 'textMessageWithHTTP200': source.textMessageWithHTTP200}, status=http.client.OK)
                else:
                    response = JsonResponse({'action': ack_flag,'config': config_flag}, status=http.client.OK)
            except Exception as E:
                return HttpResponse(status=http.client.INTERNAL_SERVER_ERROR)
        else:
            if is_external:
                response_message = {'status': http.client.OK}
                if source.textMessageWithHTTP200:
                ...
                if source.textMessageWithHTTP200:
                    response = HttpResponse(source.textMessageWithHTTP200, status=http.client.OK)
                else:
                    response = HttpResponse(status=http.client.OK)
...
def get_timeseries_data(user, starttime, endtime, exclude_streams, archived=False):
    try:
            ...
            for stream in source.fields.all():
                if stream.name not in exclude_streams:
                    ...
                    for record in data_records:
                        ...
                        if str(updated_tz) in list(data_points.keys()):
        ...
        for ts in sorted(data_points):
            ...
            for k in list(data_points[ts].keys()):
...
def mqtt_data_write(payload, request_arrival_time, source_key, user):
    try:
        ...
        if source.actuationEnabled:
            ...
            response = http.client.OK, source.textMessageWithHTTP200
        # TODO: log a data write for monitoring, logging and volume calculation - all of this would be offline process
        write_a_data_write_ttl(source.user.id, source.sourceKey,
                               source.timeoutInterval,
                               True, request_arrival_time)
