    ...
    try:
        ...
        if len(value)>0:
            ...
            try:
                ...
                for capacity, number in capacities.items():
            ...
            if device_type == 'ajb':
                ...
                if mobile_data is None:
                    if 'user-agent' in self.request.headers:
                        http_user_agent = str(self.request.headers['user-agent'])
...
def get_residual_data(starttime, endtime, plant):
    try:
        ...
        for key in sorted_nicely(list(final_residual_values.keys())):
    ...
    def create(self, request, plant_slug=None, **kwargs):
        try:
            ...
            try:
                ...
                sources_stream_association = dict(iter(self.request.data.items()))
            except Exception as exception:
                logger.debug(str(exception))
                sources_stream_association = {}

            sources_list = list(sources_stream_association.keys())
            sio = io.StringIO()
            ...
            streams_list = list(sources_stream_association_transformed.keys())
...
class PlantKPIValues(ProfileDataInAPIs, viewsets.ViewSet):
    ...
    def create(self, request, plant_slug=None, **kwargs):
        try:
            ...
            if "timestamp_type" and "count_time_period" and "identifier" and "kpi_type" and "kpi_value" not in list(payload.keys()):
            ...
            if "timestamp" in list(payload.keys()):
...
class V1_NewPredictionDataViewSet(ProfileDataInAPIs, viewsets.ViewSet):
    ...
    def create(self, request, plant_slug=None, **kwargs):
        ...
        timestamp_types_values = (value for key, value in list(settings.TIMESTAMP_TYPES.__dict__.items())
                                             if not key.startswith('__') and not callable(key))
        count_time_period_values = (value for key, value in list(settings.DATA_COUNT_PERIODS.__dict__.items())
                                             if not key.startswith('__') and not callable(key))
        try:
            context = self.get_profile_data(**kwargs)
            plants = filter_solar_plants(context)
