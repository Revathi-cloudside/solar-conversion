...
def df_for_pvsyst_analysis(st, et, user, plants):
    try:
        ...
        restructure = ['Plant', 'Location', 'Capacity (kWp)', 'Date', 'Actual Total Gen', 'Actual Irradiation',
                       'Actual PR', 'Specific Yield (Per kWp gen)',
                       'Expected Gen (As per PV syst)',
                       'Expected generation considering 2.5% for 1st yr. & 0.7% yearly  after that.',
                       'Expected Irradiation', 'Expected Gen corrected to actual irradiation',
                       'Expected PR (As per PV syst)']
        restructure2 = ['Plant', 'Location', 'Capacity (kWp)', 'Date', 'Actual Total Gen', 'Actual Irradiation',
                        'Actual PR', 'Specific Yield (Per kWp gen)']
        for key in df_dict_meta:
...
def ExcelPvsystFormat(df, ws, days_range, no_of_days):
    try:
        ...
        for i in list(range(1, 6)) + list(range(16, 20)):
