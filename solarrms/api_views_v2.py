...
class EditUserPreferences(ProfileDataInAPIs, viewsets.ViewSet):
    ...
    def list(self, request, **kwargs):
        ...
        try:
            ...
            organization_set = set(Organization.objects.filter(
                users=owner_user_id).values_list('id', flat=True))
            ...
            for usr in users_all:
                ...
                if not solar_plant_dict.get(usr['organization_id'], None):
                    ...
                    user_plant_dict[usr['user_id']]['plants'].append(
                        solar_plant_dict[usr['organization_id']])
            ...
            for usr in all_user_no_plants:
                user_plant_dict[usr['user_id']] = {"role": usr['user__role__role'],
                                                   ...
                                                   "plants": []}
            # all plants
            plants = SolarPlant.objects.filter(
                organization_ptr__in=organization_set).values('id', 'name')
    ...
    def retrieve(self, request, user_id=None, **kwargs):
        ...
        try:
            ...
            users_all = UserRole.objects.filter(
                dg_client=user_role.dg_client).values_list('user_id', flat=True)
            ...
            organization_set = set(organization_set.filter(
                users=user_id).values_list('id', flat=True))
            ...
            logger.debug(
                "Error in retrieve edituserpreferences: %s" % exception)
            return Response("INTERNAL_SERVER_ERROR", status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def partial_update(self, request, user_id=None, **kwargs):
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if user.id in users_all:
                    ...
                    if 'user_disabled' in update_data_dict_key:
                        user_role.account_suspended = int(
                            dict_changes['user_disabled'])
                    ...
                    if 'plants' in update_data_dict_key:
                        current_plants_obj = SolarPlant.objects.filter(
                            organization_ptr__in=organization_list)
                        current_plants = current_plants_obj.values_list(
                            'id', flat=True)
                        new_plants_list = dict_changes['plants']
                        # from front end it will it will come with all the plant_ids
                        if new_plants_list:
                            to_delete_plants = set(
                                current_plants) - set(new_plants_list)
                            logger.debug("To delete plants %s" %
                                         to_delete_plants)
                            to_add_plants = set(
                                new_plants_list) - set(current_plants)
                            ...
                            for add_plant in SolarPlant.objects.filter(id__in=to_add_plants):
                                user.organizations_organizationuser.create(
                                    organization_id=add_plant.id)
                        else:
            ...
            logger.debug(
                "Error in getting device details of User with user id %s" % exception)
            return Response("INTERNAL_SERVER_ERROR", status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def list(self, request, **kwargs):
        ...
        try:
            ...
            organization_set = set(Organization.objects.filter(
                users=owner_user_id).values_list('id', flat=True))
            plants = SolarPlant.objects.filter(
                organization_ptr__in=organization_set)
            ...
            for plant in plants:
                ...
                plant_current_tags = set(
                    plant.tags.values_list('name', flat=True))
                ...
                for inverter in inverters:
                    ...
                    no_of_inverters += 1
                plant_detail_dict[int(
                    plant.id)]['inverter_details'] = inverter_details
                plant_detail_dict[int(
                    plant.id)]['no_of_inverters'] = no_of_inverters
                # get meter details
                energy_meters = plant.energy_meters.all().values(
                ...
                plant_detail_dict[int(
                    plant.id)]['energy_meter_details'] = energy_meter_details
            ...
            master_dict_plant_preferences['all_available_tags'] = list(
                set(all_tags))

            return Response(data=master_dict_plant_preferences, status=status.HTTP_200_OK)
        except Exception as exception:
            logger.debug(
                "Error in retrieve editplantpreference :%s" % exception)
            return Response("INTERNAL_SERVER_ERROR", status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def retrieve(self, request, plant_id=None, **kwargs):
        ...
        try:
            try:
                ...
                Response("Wrong Query Parameter. Should be Integer",
            ...
            organization_set = set(Organization.objects.filter(
                users=owner_user_id).values_list('id', flat=True))
            plants_list = set(SolarPlant.objects.filter(
                organization_ptr__in=organization_set).values_list('id', flat=True))
            ...
            for inverter in inverters:
                ...
                for mppt in all_mppts:
                    ...
                    for string in all_strings:
                        panels_make = "%s %s" % (
                            plant.metadata.panel_manufacturer, plant.metadata.model_number)
            ...
            logger.debug(
                "Error in retrieve edit plant preference :%s" % exception)
            return Response("INTERNAL_SERVER_ERROR", status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if "inverter_details" in payload:
                    ...
                    inverter_id = list(payload['inverter_details'].keys())[0]
                    inverter_data_to_change = set(
                        payload['inverter_details'][inverter_id])
                    ...
                    if inverter_id:
                        ...
                        inverter = IndependentInverter.objects.get(
                            id=int(inverter_id))
                        ...
                        if inverter in inverters_in_plant:
                            ...
                            if 'mppt_details' in inverter_data_to_change:
                                ...
                                if "new_mppt_details" in list(inverter_payload['mppt_details'].keys()):
                                    ...
                                    mppt_already_exists = all_mppts_in_inverter.filter(
                                        name=mppt_name)
                                    if len(mppt_already_exists) > 0:
                                        logger.debug('mppt already exists ')
                                        raise Exception(
                                            "MPPT Name already exists. Please Specify Different Name")
                                    else:
                                        logger.debug(
                                            "MPPT not present. Creating New MPPT")
                                        ...
                                        if mppt:
                                            ...
                                            for i in range(strings_per_mppt):
                                                ...
                                                PanelsString.objects.bulk_create(
                                                    panel_string)
                                elif "delete_mppt" in list(inverter_payload['mppt_details'].keys()):
                                    logger.debug(
                                        "About to delete mppt ------------------")
                                    mppt_id_to_delete = inverter_payload['mppt_details']['delete_mppt']
                                    MPPT.objects.get(
                                        id=mppt_id_to_delete).delete()
                                    current_number_of_mppts = inverter.number_of_mppts
                                    inverter.number_of_mppts = current_number_of_mppts - 1
                                    logger.debug(
                                        "Deleted MPPT with ID : %s" % mppt_id_to_delete)
                                else:
                                    mppt_id_to_change = list(
                                        inverter_payload['mppt_details'].keys())[0]
                                    mppt_data_to_change = inverter_payload['mppt_details'][mppt_id_to_change]

                                    mppt_obj = MPPT.objects.get(
                                        id=int(mppt_id_to_change))

                                    if mppt_obj in all_mppts_in_inverter:
                                        logger.debug(
                                            "Changing of mppt is allowed")
                                        if 'mppt_name' in mppt_data_to_change:
                                            ...
                                            string_check = all_strings.filter(
                                                name=string_name)
                                            if len(string_check) > 0:
                                                raise Exception(
                                                    "String Name already Exists. Choose Different String Name")
                                            else:
                                            ...
                                            logger.debug(
                                                "Deleting string from mppt")
                                            delete_string_id = mppt_data_to_change['delete_string']

                                            PanelsString.objects.get(
                                                id=delete_string_id).delete()
                                            logger.debug(
                                                "String Deleted successfully, id =  %s" % delete_string_id)

                                        elif 'strings_details' in mppt_data_to_change:
                                            logger.debug(
                                                "changing string details")
                                            string_id_to_change = list(
                                                mppt_data_to_change['strings_details'].keys())[0]
                                            string_data_to_change = mppt_data_to_change['strings_details'][
                                                string_id_to_change]
                                            all_mppt_strings = mppt_obj.panels_strings.all()
                                            string_obj = PanelsString.objects.get(
                                                id=string_id_to_change)

                                            if string_obj in all_mppt_strings:
                                                logger.debug(
                                                    "change string allowed")
                                                ...
                                                if 'no_of_panels_per_string' in string_data_to_change:
                                                    string_obj.number_of_panels = string_data_to_change[
                                                        'no_of_panels_per_string']

                                                string_obj.save()
                                                logger.debug(
                                                    "string saved %s" % string_id_to_change)
                            inverter.save()
                ...
                if "energy_meter_details" in payload:
                    ...
                    meter_id = list(payload['energy_meter_details'].keys())[0]
                    ...
                    if meter_id:
                        ...
                        if meter in meters_in_plant:
                            ...
                            logger.debug(
                                "Meter not in Plants Meter list : %" % meter.name)
                ...
                if "user_basic_info" in payload:
                    logger.debug(
                        "Changing users in plant preferences %s " % plant.name)
                    ...
                    if list_of_user_ids:
                        ...
                        to_delete_users = set(
                            list_of_current_users) - set(list_of_final_users)
                        to_add_users = set(list_of_final_users) - \
                            set(list_of_current_users)
                        ...
                        logger.debug(
                            "Deleting all users for plant %s" % plant.name)
                ...
                if "group_renaming" in payload:
                    ...
                    if group_payload:
                        group_id = list(group_payload.keys())[0]
                        if int(group_id) in all_group_ids_for_this_plant:
                            try:
                                group_obj = SolarGroup.objects.get(
                                    id=int(group_id))
                            ...
                            raise Exception(
                                "GroupId does not belong to the Plant. Wrong Group ID")
                    ...
                    data_logger_device_id = create_group_payload['datalogger_id'].strip(
                    ) if 'datalogger_id' in create_group_payload else ''
                    if group_name:
                    ...
                    if delete_group_id in all_group_ids_for_this_plant:
                        try:
                            group_obj = SolarGroup.objects.get(
                                id=int(delete_group_id))
                            group_obj.delete()
                            logger.debug("Group Deleted with ID :%s " %
                                         delete_group_id)
                        except SolarGroup.DoesNotExist:
                            logger.debug("SolarGroup Doesnt exists")
                            raise Exception("Solar group doesnt exist")
                    ...
                    group_key = list(add_inverter_payload.keys())[0]
                    ...
                    group_key = list(remove_inverter_payload.keys())[0]
                    inverter_id_to_remove = int(
                        remove_inverter_payload[group_key])

                    all_plant_inverters = set(
                        plant.independent_inverter_units.all().values_list('id', flat=True))
                    all_plant_groups = list(
                        plant.solar_groups.all().values_list('id', flat=True))
                    if int(group_key) in all_plant_groups and inverter_id_to_remove in all_plant_inverters:
                        try:
                            group_obj = SolarGroup.objects.get(
                                id=int(group_key))
                            inverter_obj_to_remove = IndependentInverter.objects.get(
                                id=inverter_id_to_remove)
                            group_obj.groupIndependentInverters.remove(
                                inverter_obj_to_remove)
                    ...
                    logger.debug(
                        "Altering inverter group assocciation +++++_--------*****/////")
                    alter_i_g_payload = payload['alter_inverter_group_association']
                    inverter_key = list(alter_i_g_payload.keys())[0]

                    remove_inverter_from_this_group_key = int(
                        alter_i_g_payload[inverter_key]['from'])
                    add_inverter_to_this_group_key = int(
                        alter_i_g_payload[inverter_key]['to'])
                    ...
                    if int(remove_inverter_from_this_group_key) in all_plant_groups and int(inverter_key) in all_plant_inverters:
                        try:
                            inverter_obj = IndependentInverter.objects.get(
                                id=inverter_key)

                            remove_inverter_from_this_group_obj = SolarGroup.objects.get(
                                id=int(remove_inverter_from_this_group_key))
                            remove_inverter_from_this_group_obj.groupIndependentInverters.remove(
                                inverter_obj)

                            add_inverter_to_this_group_obj = SolarGroup.objects.get(
                                id=int(add_inverter_to_this_group_key))
                            add_inverter_to_this_group_obj.groupIndependentInverters.add(
                                inverter_obj)
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if 'old_password' in update_data_dict_key and 'new_password' in update_data_dict_key\
                    ...
                    user = authenticate(
                        username=user.username, password=payload['old_password'])
                    if user:
...
class UserSolarPlantWigetsConfigView(ProfileDataInAPIs, viewsets.ViewSet):
    ...
    def list(self, request, id=None, **kwargs):
        ...
        try:
            ...
            try:
                ...
                plants = filter_solar_plants(context)
            results = UserSolarPlantWidgetsConfig.objects.filter(
                user=self.request.user)
            payload = {}
            for result in results:
                payload[result.id] = {}
                payload[result.id]['config_name'] = result.config_name
                payload[result.id]['plants'] = result.plants.all().values(
                    'name', 'slug')
            ...
            if plants_meta:
                ...
                for plant in plants:
                    ...
                    plant_meta['roof_names'] = plant.solar_groups.all().values_list(
                        'name', flat=True) if plant.solar_groups.all() else []
                    plant_meta['tags'] = plant.tags.values_list(
                        'name', flat=True) if plant.tags else []
            ...
            logger.debug(
                "Error in list UserSolarPlantWigetsConfigView: %s" % exception)
            return Response("INTERNAL_SERVER_ERROR", status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def retrieve(self, request, id=None, **kwargs):
        ...
        try:
            ...
            logger.debug(
                "Error in list UserSolarPlantWigetsConfig: %s" % exception)
    ...
    def partial_update(self, request, id=None, **kwargs):
        ...
        try:
            ...
            try:
                ...
                plants = filter_solar_plants(context)
            try:
                current_upws = UserSolarPlantWidgetsConfig.objects.get(
                    id=int(id))
            ...
            with transaction.atomic():
                if 'delete' in payload:
                    try:
                        UserSolarPlantWidgetsConfig.objects.get(
                            id=int(payload['delete'])).delete()
                    except Exception:
            ...
            for result in results:
                ...
                return_payload[result.id]['plants'] = result.plants.all().values(
                    'name', 'slug')
        ...
        try:
            ...
            try:
                with transaction.atomic():
                    ...
                    all_plants = SolarPlant.objects.filter(
                        slug__in=new_plant_slugs)
            ...
            logger.debug(
                "Error in list user solar plant widget: %s" % exception)
            return Response("INTERNAL_SERVER_ERROR", status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class UserTableWidgetsConfigView(ProfileDataInAPIs, viewsets.ViewSet):
    ...
    def list(self, request, id=None, **kwargs):
        ...
        try:
            ...
            try:
                config = UserTableWidgetsConfig.objects.get(
                    user=user, config_scope=config_scope)
            except Exception as e:
                logger.debug("couldnt find config, sending default config")
                # data_key_name is type of feature
                all_columns = accessible_features.features.all().filter(
                    include_table=True).values('id', 'name', 'data_key_name')

                enabled_features_list = []
                all_features_list = []
                payload_to_send = {}
                elist = {'ENERGY_GENERATION': 1,
                         'CURRENT_POWER': 2, 'SY': 3, 'CUF': 4}
                enable_order_id = 0
                for col in all_columns:
                    if col['name'] in elist:
                        ...
                        enabled_features['text'] = FEATURE_DETAILS["%s" %
                                                                   col['name']]['text']
                        ...
                        all_features['text'] = FEATURE_DETAILS["%s" %
                                                               col['name']]['text']
            ...
            all_columns = accessible_features.features.all().filter(
                include_table=True).values('name', 'id')
            ...
            for fixed_feature in fixed_features:
                ...
                fixed_feature['feature_name'] = fixed_feature.pop(
                    'features__name')
                fixed_feature['text'] = FEATURE_DETAILS["%s" %
                                                        fixed_feature['feature_name']]['text']
                fixed_feature['type'] = fixed_feature.pop(
                    'features__data_key_name')
            temp_dict = {}
            ...
            for enabled_feature in enabled_features:
                ...
                enabled_feature['feature_id'] = enabled_feature.pop(
                    'features__id')
                enabled_feature['feature_name'] = enabled_feature.pop(
                    'features__name')
                enabled_feature['text'] = FEATURE_DETAILS["%s" %
                                                          enabled_feature['feature_name']]['text']
                enabled_feature['type'] = enabled_feature.pop(
                    'features__data_key_name')
            temp_dict = {}
            ...
            remaining_columns = set(
                all_available_columns.keys()) - set(assigned_columns)
    ...
    def create(self, request, id=None, **kwargs):
        ...
        try:
            ...
            try:
                config, flag = UserTableWidgetsConfig.objects.get_or_create(
                    user=user, config_scope=config_scope)
            except Exception as e:
            ...
            if not new_fixed_features == 'none':
                ...
                if flag == False:
                    fixed_features = FeatureOrder.objects.filter(
                        user_table_widgets=config, is_fixed=True)
                    if fixed_features:
            ...
            if not new_enabled_features == 'none':
                ...
                if flag == False:
                    enabled_features = FeatureOrder.objects.filter(
                        user_table_widgets=config, is_fixed=False)
                ...
                if not new_enabled_features == []:
                    ...
                    for feature_id in new_enabled_features:
                        ...
                        order_number = order_number + 1
                    FeatureOrder.objects.bulk_create(
                        new_enabled_features_to_add)
...
class PlantGroupView(ProfileDataInAPIs, viewsets.ViewSet):
    ...
    def list(self, request, plant_slug=None, **kwargs):
        try:
            ...
            logger.debug("Error in getting device details of plant : " +
...
class V2_NewPredictionDataViewSet(viewsets.ViewSet):
    ...
    def create(self, request, plant_slug=None, **kwargs):
        ...
        """
        model_name_values = (
            'STATISTICAL_DAY_AHEAD', 'STATISTICAL_LATEST', 'ACTUAL_VALUE', 'ACTUAL', 'LSTM')
        timestamp_types_values = tuple(value for key, value in list(settings.TIMESTAMP_TYPES.__dict__.items())
                                       if not key.startswith('__') and not callable(key))
        count_time_period_values = tuple(value for key, value in list(settings.DATA_COUNT_PERIODS.__dict__.items())
                                         if not key.startswith('__') and not callable(key))
        try:
            try:
                plant = SolarPlant.objects.get(slug=plant_slug)
            ...
            payload = [t_post_data] if isinstance(
                t_post_data, dict) else t_post_data
            ...
            for post_data in payload:
                ...
                try:
                    NewPredictionData.objects.create(timestamp_type="%s" % post_data['timestamp_type'],
                                                     count_time_period=int(
                                                         post_data['count_time_period']),
...
class GECleaningTrigger(ProfileDataInAPIs, viewsets.ViewSet):
    ...
    def list(self, request, plant_slug=None, **kwargs):
        ...
        if channel == 'ui':
            ...
            all_string_name = {}
            for ajb in all_ajbs:
                stream_names = ajb.fields.filter(
                    isActive=True, name__in=string_name).values_list('name', flat=True)
                ...
                for stname in stream_names:
                    if stream_exists and stname in stream_exists:
                        continue
                    all_string_name['%s+%s' %
                                    (ajb.sourceKey, stname)] = "%s (%s)" % (ajb.name, stname)
    ...
    def create(self, request, plant_slug=None, **kwargs):
        ...
        try:
            ...
            if trigger_type in list(dict(TRIGGER_TYPE).keys()) and present_state in list(dict(CLEANING_PRESENT_STATE).keys()):
                print("ok")
            else:
                return Response(
            ...
            if channel == 'em':
                ...
                if present_state == "in_process":
                    if ct[0].present_state == "open":
                        ct.update(present_state="in_process",
                ...
                if present_state == "finished":
                    if ct[0].present_state == "in_process":
                        ct.update(present_state="finished",
    ...
    def list(self, request, format_id=None, **kwargs):
        ...
        try:
            ...
            if not user.role.enable_preference_edit:
                return Response("Not Authorized", status=status.HTTP_401_UNAUTHORIZED)
            client = self.request.user.role.dg_client
            all_customised_format = CustomReportFormat.objects.filter(
                dg_client=client)
            ...
            for format in all_customised_format:
                ...
                single_format = {'custom_format_name': format.name, 'is_default': format.role_default, 'role': format.role,
                                 'custom_features': custom_features, 'users_list': users_list, 'custom_format_id': format.id}
            ...
            pdf_features = list(feature_dynamic_pdf_ids_mapping.values())
            ...
            for role_access in all_role_accesses:
                ...
                for feature_obj in user_features_obj_list:
                    user_features.append(
                        {"feature_id": feature_obj.id, "feature_name": str(feature_obj)})
                ...
                role_users_dict[role_access.role] = UserRole.objects.filter(
                    dg_client=client, role=role_access.role).values('user_id', 'user__email')
    ...
    def partial_update(self, request, format_id=None, **kwargs):
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    "Payload problems in customreportformatview partial update %s" % exception)
                return Response("Please specify payload in request body", status=status.HTTP_400_BAD_REQUEST)
            ...
            try:
                ...
                logger.debug(
                    "No format found in customreportformat %s" % exception)
                return Response("format is not found", status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            if 'users_list' in payload:
                all_users = UserRole.objects.filter(
                    dg_client=client, role=c_format.role).values_list('user_id', flat=True)
                ...
                if set(new_users_list).issubset(set(all_users)):
                    ...
                    to_delete_users = set(
                        current_users_list)-set(new_users_list)
                    to_add_users = set(new_users_list) - \
                        set(current_users_list)
                    for user_id_r in to_delete_users:
                        try:
                            c_format.user.remove(
                                User.objects.get(id=user_id_r))
                        except Exception as exception:
            ...
            if 'make_default_for_role' in payload:
                try:
                    default_format = CustomReportFormat.objects.get(
                        dg_client=client, role=c_format.role, role_default=True)
    ...
    def create(self, request, id=None, **kwargs):
        ...
        try:
            ...
            if set(["name", "role"]).issubset(set(payload.keys())) and payload["list_of_new_feature_dict"]:
                check_if_similar_report_format_exists = CustomReportFormat.objects.filter(
                    name=payload['name'], dg_client=dg_client, role=payload['role'])
                ...
                c_format = CustomReportFormat.objects.create(
                    name=payload['name'], dg_client=dg_client, role=payload['role'])
                updation_reply = c_format.update_custom_format(
                    payload["list_of_new_feature_dict"])
                if 'make_default_for_role' in payload:
                    try:
    ...
    def list(self, request, plant_slug=None, **kwargs):
        try:
            try:
                user_role = self.request.user.role.role
                user_client = self.request.user.role.dg_client
                plant = SolarPlant.objects.get(
                    slug=plant_slug, groupClient=user_client)
                if str(user_role) != 'CEO':
                    # verify_if_user_has_plant_access
                    if not (plant.users.get(id=self.request.user.id)):
                        logger.debug(
                            "Not Authorised for this plant slug %s" % plant_slug)
                        return Response("Not Authorised for this plant slug %s" % plant_slug,
                                        status=status.HTTP_401_UNAUTHORIZED)

            except Exception as exception:
                logger.debug("Something went wrong slug = %s , >>%s" %
                             (plant_slug, exception))
            ...
            with open('%s' % file_name, 'rb') as pdf:
                ...
                response = HttpResponse(
                    pdf.read(), content_type='application/force-download')
...
class PVSystAchievementReportDownload(ProfileDataInAPIs, viewsets.ViewSet):
    ...
    def list(self, request, id=None, **kwargs):
        ...
        try:
            ...
            workbook = cron_send_pvsyst_report_cleanmax(
    ...
    def create(self, request, **kwargs):
        ...
        try:
            ...
            try:
                ...
                all_plants_accessible = SolarPlant.objects.filter(
                    groupClient=user.role.dg_client).values_list('slug', flat=True)
            ...
            plant_slugs = payload
            workbook = cron_send_pvsyst_report_cleanmax(
                startdate, enddate, user, plant_slugs)
            try:
...
class MonthlyGenerationBill(ProfileDataInAPIs, viewsets.ViewSet):
    ...
    def list(self, request, plant_slug=None, **kwargs):
        try:
            ...
            try:
                plant = SolarPlant.objects.get(
                    slug=plant_slug, groupClient=user.role.dg_client)
            except Exception as exception:
                logger.debug(
                    "Invalid Plant Slug entered in monthlygenerationbill %s" % exception)
        ...
        try:
            ...
            for plant in plants:
                ...
                for key in inverters_dict:
                    no_of_inverters = no_of_inverters + \
                        str(inverters_dict[key]) + " - " + key + " "
