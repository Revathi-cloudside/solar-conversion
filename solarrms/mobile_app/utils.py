...
def get_client_owner_status_data_mobile_OandM_Manager(client, current_time, solar_plants,role):
    try:
        ...
        if len(plant_groups)>0:
            for key in list(plant_groups.keys()):
                try:
                    plant_group = {}
...
def get_plant_status_mobile_api(plant, date, role, combined=False):
    try:
        ...
        try:
            plant_values['gateways_disconnected'] = len(list(plant_network_details['GATEWAY_DISCONNECTED']['details'].keys()))
        except:
            plant_values['gateways_disconnected'] = 0
            pass
        try:
            plant_values['gateways_powered_off'] = len(list(plant_network_details['GATEWAY_POWER_OFF']['details'].keys()))
        except:
