...
class UserRole(models.Model):
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name="role", related_query_name="role")
    role = models.CharField(max_length=100, null=False, blank=False, choices=SOLAR_USER_ROLE)
    dg_client = models.ForeignKey(DataglenClient, on_delete=models.CASCADE, null=True, blank=True, related_name="dg_client", related_query_name="dg_client")
...
class UserRolePlantsAlertsPreferences(models.Model):
    user_role = models.ForeignKey(UserRole, on_delete=models.CASCADE, related_name="alerts_preferences_plants",
...
class AlertsCategory(models.Model):
    user_role = models.ForeignKey(UserRole, on_delete=models.CASCADE, related_name="alerts_preferences", related_query_name="alerts_preferences")
        ...
        for event_type in EVENT_TYPE:
            try:
                with transaction.atomic():
                    if event_type[0] in list(DEFAULT_THRESHOLD.keys()):
