...
def get_ajbs_live_status(xxx_todo_changeme):
    (plant, ajbs) = xxx_todo_changeme
    try:
...
def get_inverters_power_or_energy_data(xxx_todo_changeme1):
    #logger.debug("starting a new process for get_inverters_power_or_energy_data : " + str(len(inverters)))
    # restart django's connection with mysql
    (plant, inverters, energy, starttime, endtime) = xxx_todo_changeme1
    from django.db import connection
    try:
        if connection.connection:
...
def get_inverters_live_status(xxx_todo_changeme2):
    (plant, inverters, inverters_generation, mobile_app) = xxx_todo_changeme2
    try:
...
def generation_grouping(xxx_todo_changeme3):
    #logger.debug("starting a new process for energy aggregation information : " + str(len(devices)))
    # restart django's connection with mysql
    (plant, devices, df, original_starttime, original_endtime, aggregation, local_timezone, meter_energy) = xxx_todo_changeme3
    from django.db import connection
    try:
        if connection.connection:
            connection.connection.close()
    ...
    for device in devices:
        try:
            ...
            if not df_device.empty:
                ...
                energy = grouped.sum()
                for key, value in energy[device.name].items():
                    try:
