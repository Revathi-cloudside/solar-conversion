...
def get_live_chart_data(identifiers):
    try:
        ...
        assert(len(list(valid_records_second.keys())) == len(list(invalid_records_second.keys())))

        valid_records = []
        invalid_records = []

        for key, value in sorted((iter(valid_records_second.items()))):
        ...
        for key, value in sorted((iter(invalid_records_second.items()))):
