...
class TimeStampedModelMixin(models.Model):
    ...
    """
    # Timestamp
    created = models.DateTimeField("Date created", auto_now_add=True, db_index=True)
...
class Event(models.Model):
    event_category = models.CharField(choices=list(zip(event_categories, event_categories)),
                                      max_length=50, blank=False, null=False)
    priority_category = models.CharField(choices=list(zip(priority_categories, priority_categories)),
                                         max_length=50, blank=False, null=False)
...
class ContractState(BaseState):
    contract = models.ForeignKey(MaintenanceContract, on_delete=models.CASCADE, related_name="contract_states")
...
class ServiceRequest(models.Model):
    contract = models.ForeignKey(MaintenanceContract, on_delete=models.CASCADE, related_name="service_requests")
    plant = models.ForeignKey(SolarPlant, on_delete=models.CASCADE, related_name="service_requests")
    priority = models.CharField(choices=list(zip(priority_categories, priority_categories)),
                                max_length=100, null=False, blank=False)
    event = models.OneToOneField(Event, on_delete=models.CASCADE, related_name="service_request")
    sla_met = models.BooleanField(default=True, blank=False, null=False)
    closed_at = models.DateTimeField(
        "Closing time", db_index=True, blank=True, null=True)
...
class ServiceRequestState(BaseState):
    service_request = models.ForeignKey(ServiceRequest, on_delete=models.CASCADE, related_name="service_states")
...
class SLA(TimeStampedModelMixin):
    contract = models.ForeignKey(MaintenanceContract, on_delete=models.CASCADE, related_name="slas")
    priority = models.CharField(choices=list(zip(priority_categories, priority_categories)),
                                max_length=50, blank=False)
    from_state = models.ForeignKey(ContractState, on_delete=models.CASCADE, related_name="from_slas")
    to_state = models.ForeignKey(ContractState, on_delete=models.CASCADE, related_name="next_slas")
...
class OpeningHours(models.Model):
    ...
    contract = models.ForeignKey(MaintenanceContract, on_delete=models.CASCADE, verbose_name=_('MaintenanceContract'))
