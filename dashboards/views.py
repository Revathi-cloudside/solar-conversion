...
class EntryPointPostLogin(EntryPointMixin, View):
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.user_status is settings2.USER_STATUS.OWNER:
            return redirect(reverse(self.dashboard.ownerViewURL))
        elif self.user_status is settings2.USER_STATUS.EMPLOYEE:
            return redirect(reverse(self.dashboard.employeeViewURL))
        elif self.user_status is settings2.USER_STATUS.MEMBER:
            return redirect(reverse(self.dashboard.groupViewURL))
        elif self.user_status is settings2.USER_STATUS.UNASSIGNED:
